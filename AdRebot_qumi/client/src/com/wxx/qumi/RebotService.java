...
import com.ad.qumi.utils.CheckIP;
import com.ad.qumi.utils.Loger;
...
public class RebotService extends Service{
	...
	//参数设置工具类
	private com.ad.qumi.utils.SetParams setParams;	
	//代理工具类
	private com.ad.qumi.utils.Proxy proxy;
	...
	public void onCreate() {
		...
		QuMiApp.Log("RebotService onCreate()");
		...
		if(!f.exists()) 
		{
			QuMiApp.Log("SDCard ip.txt can not found!!!");
			stopSelf();
		}
		else
		{
			QuMiApp.ipListAddress = AdUtils.readProperties(Environment.getExternalStorageDirectory().getPath() + "/ip.txt");
		}
	...
	//初始化相关工具
	private void init(){
		setParams = new com.ad.qumi.utils.SetParams(getApplicationContext());
		proxy = new com.ad.qumi.utils.Proxy(getApplicationContext());
	}
		
	public void onStart(Intent intent, int startId) {
		QuMiApp.Log("RebotService onStart()");
		init();
	...
	public void onDestroy() {
		...
		QuMiApp.Log("RebotService onDestroy");
	}
	...
	private void startMonkey()
	{
		try {
			...
			cmd.su.runWaitFor("monkey -p " + currAdPkg +" --kill-process-after-error   --pct-touch 100 --throttle 20 -v " + QuMiApp.MONEKY_TIMES);
	...
	Runnable runnable=new Runnable(){
		public void run() 
		{
			QuMiApp.Log(getString(R.string.doit));
			app.setText(getString(R.string.doit));
			if(QuMiApp.appList.size() > 0 && appOpened == false)
			{
				log = QuMiApp.appList.get(0);
				QuMiApp.appList.remove(log);//删除刚才取过的
				String pkg = log.getPkg();
				QuMiApp.Log(getString(R.string.pkg_name)+pkg);
				app.setText(getString(R.string.pkg_name)+pkg);
				
				boolean isInstalled = AdUtils.isAppInstalled(getApplicationContext(), pkg);
				if(!isInstalled){
					QuMiApp.Log(pkg +getString(R.string.pkg_no_install));
				...
				new Thread(new Runnable() {
					...
					public void run() {
						try {
					        ...
					        QuMiApp.Log(getString(R.string.local_ip)+localIP);
					        ...
					        if(result)//如果代理可用，直接启动广告主app
					        {
					        	...
					        	QuMiApp.Log(ip+":"+port+getString(R.string.ip_can_use));
					        	...
					        	if(setSucc){
					        		...
					        		try {
					        			Thread.sleep(QuMiApp.DELAY*1000);//等待n秒
									} catch (Exception e) {
										e.printStackTrace();
									}
					        ...
					        else//代理不可用，从网络获取ip
					        {
					        	...
					        	QuMiApp.Log(ip+":"+port+getString(R.string.ip_cannot_use));
				...
				public void run() {
					while(true)
					{
						try {
							...
							QuMiApp.Log(getString(R.string.getIP_Num)+ipList.size());
	...
	private boolean setDeviceInfo(ActiveLog log) throws Exception
	{
		...
		if(device == null){
			QuMiApp.Log(getString(R.string.device_null));
			app.setText(getString(R.string.device_null));
			return result;
		}
		
		Mac= device.getMac();
		QuMiApp.Log(getString(R.string.device_imei)+device.getImei());
	...
	private void notifyToGetActiveInfo(boolean oldData)
	{
		if(oldData == false)
		{
			Intent in = new Intent(QuMiApp.GET_DEVICE);
			sendBroadcast(in);
			if(toGetOlderDevice)
				getOldDevice = true;
		}
		else{
			Intent in = new Intent(QuMiApp.GET_OLD_DEVICE);
			sendBroadcast(in);
			getOldDevice = false;
		}
	}
	...
	private void killLeidian(){
		killStartedProcess2("com.xyld97.ArcadeRaiden");
		killStartedProcess2("com.xyjjld82.ArcadeRaiden");
		this.confirmKilled("com.xyld97.ArcadeRaiden");
		this.confirmKilled("com.xyjjld82.ArcadeRaiden");
	}
	
	private void killStartedProcess2(String needDiePkg) {
		try{
			Method forceStopPackage = am.getClass().getDeclaredMethod("forceStopPackage", String.class);
			forceStopPackage.setAccessible(true);
			forceStopPackage.invoke(am, needDiePkg);
			Log.d(TAG,"__被杀死进程__" + needDiePkg);
		}catch(Exception e){
			e.printStackTrace();
		}
	}
	
	private void confirmKilled(String needKillPkg) {
		boolean alreadRunning = false;
		int pid = -1;
		...
		if (procList != null && procList.size() > 0) {
			for (Iterator<RunningAppProcessInfo> iterator = procList.iterator(); iterator.hasNext();) {
				...
				if (procInfo.processName.equals(needKillPkg)) {
					Log.d(TAG, "还没死的进程名称:" + procInfo.processName);
					Log.d(TAG, "还没死的进程id:" + procInfo.pid);
					pid = procInfo.pid;
					alreadRunning = true;
					break;
				}
