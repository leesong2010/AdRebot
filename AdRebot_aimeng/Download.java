...
class Download extends Observable{
    ...
    public Download(URL url,String UA,MyIP myip,boolean trueDown) {
        this.url = url;
        size = -1;
        downloaded = 0;
        status = DOWNLOADING;
    ...
    public String toDownload() throws Exception
    {
        ...
        try {
         ...
         //   cancel();
            ...
            if(trueDown){
	            File dir = new File(AppContact.FILESAVEPATH);
	            if(!dir.exists())dir.mkdirs();
	            file = new RandomAccessFile(AppContact.FILESAVEPATH + tmpFileName, "rw");//TODO 这里改名字
	            ...
	            adv =  Utils.getApkInfo(AppContact.FILESAVEPATH+tmpFileName, AppContact.AAPTPATH);
            }
            else{
            	stream = connection.getInputStream();
            	byte buffer[];
                ...
                int minSpeed = Integer.parseInt(AppContact.DOWN_SPEED.split(",")[0]);
                int maxSpeed = Integer.parseInt(AppContact.DOWN_SPEED.split(",")[1]);
        	...
        	File f = new File(AppContact.FILESAVEPATH + tmpFileName);
        	if(f.exists()){
