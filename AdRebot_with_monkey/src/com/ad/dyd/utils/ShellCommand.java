...
public class ShellCommand {
    ...
    //获取进程ID
  	public static int getProcessPid(Context ctx,final String pkgName) {
  	    ...
  	    if(procList != null && procList.size() > 0)
  	    {
	  	    for (Iterator<RunningAppProcessInfo> iterator = procList.iterator(); iterator.hasNext();) 
	  	    {
	  	        ...
	  	        if(procInfo.processName.equals(pkgName))
	  	        {  
	  		        Log.d(TAG,"proc_name:"+procInfo.processName);
	  		        Log.d(TAG,"proc_id:"+procInfo.pid);
	  	        	result=procInfo.pid;
	  	        	break;
	  	        } 
	  	    }
