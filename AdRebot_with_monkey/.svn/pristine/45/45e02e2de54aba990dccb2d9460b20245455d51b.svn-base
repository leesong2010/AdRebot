...
public class Loger {
	...
	private static File logFile = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + "/adrebot/qumi.txt");
	...
	static {
		try {
			if (write) {
				if (!logFile.exists()) {
					File f = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + "/adrebot/");
					if(!f.exists())f.mkdir();
